...
对全部中文候选文档做一次字符级扫描。对每个新增汉字 \(c\) 统计：
...
产物：`new_hanzi_frequency.parquet`
...
使用 `Unique Hanzi` 而不是字符出现次数，是为了防止某个汉字在同一文档重复很多次后获得异常高分。权重：
...
除新增 token 外，还需要避免 PGCA 某些特征长期训练不足。统计全候选语料中的：
...
在来源配额和Token预算内优先解决新增 token 覆盖不足问题。
...
覆盖诊断参考（不参与 passed，不要求全部达标后才填充配额）：
...
主实验仍无法找到时记录缺口，不生成文本。仅独立消融允许少量构造 coverage sample，必须满足：
...
### 8. 第二轮——按优先级补齐到目标 token 数

完成预算内覆盖优先选择后，对剩余高质量文档按 \(S(d)\) 排序补齐。

目标：

\[
T_{enhance}=0.10T_{Phase2}
\]
...
这些约束针对增强池内部，而不是整个 Phase2。目的是让新增 token 同时学习：
...
对于新增 token 中的多音字，额外统计其上下文多样性。例如一个字符有：

\[
P(c)=\{p_1,p_2,\ldots,p_k\}
\]

不在数据中写入“正确拼音标签”。只选择多样化自然上下文，例如：
...
同一父文档只能出现在最终 Phase2 mixture 一次。否则10%增强配额会变成重复训练，而不是独立的数据组成。

流程顺序：

**增强池选择 → 全局 doc_id 去重 → 原 bucket 删除 → 最终 mixture 合并。**

### 12. 最终精确 Tokenization

前面所有筛选都基于 Unicode 字符和文档统计，不需要执行 tokenizer。只有增强候选最终确定后：
...
该阶段完成后按下表验收；覆盖率三项仅作诊断：

| 指标 | 验收要求 |
|---|---:|
| 总量 | 1B tokens ±1% |
| 自然文本比例 | ≥99% |
| 新增汉字有自然上下文覆盖 | 参考99%，非硬门槛 |
| 新增汉字 ≥20 个文档覆盖 | 参考95%，非硬门槛 |
| 新增汉字 ≥100 个文档覆盖 | 参考90%，非硬门槛 |
| 单一数据源 | ≤30% |
| 现代中文 | ≥40% |
| 人工补充 | ≤1% |
| Exact duplicate | 0 |
