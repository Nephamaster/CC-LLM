...
class Qwen3PGCADecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3PGCAConfig, layer_idx: int):
        ...
        self.pgca_attn = None
        if getattr(config, "use_pgca", False) and layer_idx in set(getattr(config, "pgca_layers", []) or []):
            self.pgca_attn = PGCACrossAttention(config, rms_norm_cls=Qwen3RMSNorm)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.pgca_attn is not None and feature_memory is not None:
            pgca_output = self.pgca_attn(normed_hidden_states, feature_memory, feature_mask)
        else:
            pgca_output = 0
        hidden_states = residual + self_attn_output + pgca_output
...
class Qwen3PGCAModel(Qwen3PGCAPreTrainedModel):
    def __init__(self, config: Qwen3PGCAConfig):
        ...
        self.feature_memory_builder = _build_feature_memory_builder(config)
    ...
    def _prepare_feature_memory(
        ...
        feature_ids: dict[str, torch.Tensor] | None,
        feature_memory: torch.Tensor | None,
        feature_mask: torch.Tensor | None,
    ) -> tuple[torch.Tensor | None, torch.Tensor | None]:
        if feature_memory is not None:
            if feature_mask is None:
                raise ValueError("feature_mask must be provided when feature_memory is provided")
        elif feature_ids is not None:
            if self.feature_memory_builder is None:
                raise ValueError("feature_ids require an initialized feature_memory_builder")
            feature_memory, feature_mask = self.feature_memory_builder.feature_embedding(feature_ids)
        elif input_ids is not None and self.feature_memory_builder is not None:
            feature_memory, feature_mask = self.feature_memory_builder(input_ids)
        else:
            return None, None

        seq_len = inputs_embeds.shape[1]
        if feature_memory.shape[1] != seq_len:
            if feature_memory.shape[1] < seq_len:
                raise ValueError("feature_memory sequence length is shorter than hidden states")
            feature_memory = feature_memory[:, -seq_len:, :, :]
            feature_mask = feature_mask[:, -seq_len:]

        feature_memory = feature_memory.to(device=inputs_embeds.device, dtype=inputs_embeds.dtype)
        feature_mask = feature_mask.to(device=inputs_embeds.device, dtype=torch.bool)
        return feature_memory, feature_mask
    ...
    def forward(
        ...
        feature_ids: dict[str, torch.Tensor] | None = None,
        feature_memory: torch.Tensor | None = None,
        feature_mask: torch.Tensor | None = None,
        **kwargs: Unpack[TransformersKwargs],
        ...
        feature_memory, feature_mask = self._prepare_feature_memory(
            input_ids=input_ids,
